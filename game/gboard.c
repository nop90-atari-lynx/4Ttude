...
signed char	coordtoint(signed char x, signed char y, signed char z) {
...
char	xcoord(signed char spt) {
	if ((spt >= NUM_SQUARES)||(spt < 0))
		return -1;
	return spt % NUM_ON_SIDE;
}

char	ycoord(signed char spt) {
	if ((spt >= NUM_SQUARES)||(spt < 0))
		return -1;
	return (spt / NUM_ON_SIDE)%NUM_ON_SIDE;
}

char	zcoord(signed char spt) {
	if ((spt >= NUM_SQUARES)||(spt < 0))
		return -1;
	return (spt / (NUM_ON_SIDE*NUM_ON_SIDE));
}

bool	legal(LPGBOARD brd, GB_PIECE who, signed char where) {
...
void	gb_place(LPGBOARD brd, GB_PIECE who, signed char where) {
/*
...
bool	inuse(LPGBOARD brd, signed char where) {
	if ((where < 0)||(where >= NUM_SQUARES))
		return true;
...
GB_PIECE pieceat(LPGBOARD brd, signed char where) {
	if ((where < 0)||(where >= NUM_SQUARES))
		return	GB_NOONE;
