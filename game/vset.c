...
bool	vs_isable(LPVSET vs, signed char spot) {
	if ((spot < 0)||(spot >= NUM_SQUARES))
		return false;
	return (vs->m_data[spot] > 0)?true:false;
}

void	vs_addscore(LPVSET vs, signed char spot, int delta) {
	if ((spot < 0)||(spot >= NUM_SQUARES))
		return;
	if (vs->m_data[spot] == 0)
		vs->m_active++;
...
void	vs_incscore(LPVSET vs, signed char spot) {
	vs_addscore(vs, spot, 1);
}


void	vs_subscore(LPVSET vs, signed char spot, int delta) {
...
void	vs_decscore(LPVSET vs, signed char spot) {
	vs_subscore(vs, spot, 1);
}


void	vs_disable(LPVSET vs, signed char spot) {
	if ((spot < 0)||(spot >= NUM_SQUARES))
		return;
	if (vs->m_data[spot] != 0)
		vs->m_active--;
...
signed char	vs_pickmember(LPVSET vs) {
	int	highscore;
	char cnt, chosen, i;
...
//	chosen = rand() % vs->m_active;
	chosen = ((unsigned int) clock()) % vs->m_active;
	cnt = 0;
