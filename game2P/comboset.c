...
void	cs2_init(LPCOMBOSET cs) {
	...
	for(i=0; i<16; i++)
		cr2_init(&cs->m_data[idx++], i*4, i*4+1, i*4+2, i*4+3);
	for(i=0; i<16; i++)
		cr2_init(&cs->m_data[idx++],
			(i&3)+(i&12)*4,
			(i&3)+(i&12)*4+4,
			(i&3)+(i&12)*4+8,
			(i&3)+(i&12)*4+12);
	for(i=0; i<16; i++)
		cr2_init(&cs->m_data[idx++], i, i+16, i+32, i+48);

	// Our six types of four
	for(i=0; i<4; i++)
		cr2_init(&cs->m_data[idx++], i*16, i*16+5, i*16+10, i*16+15);
	for(i=0; i<4; i++)
		cr2_init(&cs->m_data[idx++], i*16+3, i*16+6, i*16+9, i*16+12);
	for(i=0; i<4; i++)
		cr2_init(&cs->m_data[idx++], i, i+20, i+40, i+60);
	for(i=0; i<4; i++)
		cr2_init(&cs->m_data[idx++], i+12, i+24, i+36, i+48);
	for(i=0; i<4; i++)
		cr2_init(&cs->m_data[idx++], i*4, i*4+16+1, i*4+32+2, i*4+48+3);
	for(i=0; i<4; i++)
		cr2_init(&cs->m_data[idx++], i*4+3, i*4+16+2, i*4+32+1, i*4+48);

	// The last of the fours: the four diagonal, diagonals.
	cr2_init(&cs->m_data[idx++],  0,    16+4+1,    32+8+2,     48+12+3);
	cr2_init(&cs->m_data[idx++],  3,  3+16+4-1,  3+32+8-2,  3+ 48+12-3);
	cr2_init(&cs->m_data[idx++], 12, 12+16-4+1, 12+32-8+2, 12+ 48-12+3);
	cr2_init(&cs->m_data[idx++], 15, 15+16-4-1, 15+32-8-2, 15+ 48-12-3);
 ...
 */
static void
cs2_swap(LPCOMBOROW a, LPCOMBOROW b)
{
	COMBOROW	tmp;
 ...
 */
bool
cs2_place(LPCOMBOSET cs, GB_PIECE who, signed char where)
{
	signed char	i;
	...
	for(i=0; i < cs->m_ninplay; i++) {
		...
		if (cr2_register(&cs->m_data[i], who, where)) {
